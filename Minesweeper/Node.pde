...
public class Node {
  ...
  /// Returns an array of children nodes that contain a specified point.
  public ArrayList<Node> childNodesAtPoint(Point point) {
    // Create a new array.
    ArrayList<Node> matchingChildren = new ArrayList<Node>();

    // Enumerate the children array.
    for (Node child : children()) {
      // If the frame contains the point, we've found a match so add it to the collection.
      if (child.frame().containsPoint(point)) {
        matchingChildren.add(child);
      }
    }
  ...
  public void mouseDownAtPoint(Point point) {
    for (Node child : childNodesAtPoint(point)) {
      child.mouseDownAtPoint(point);
    }
  }
